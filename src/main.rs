...
fn startup(mut commands: Commands,  windows: ResMut<Windows>) {
    ...
    for _ in 0..AMOUNT {
        ...
        let dots = Dots {pos: (rng.gen::<f32>()*win.width()-(win.width()/2.0),rng.gen::<f32>()*win.height()-(win.height()/2.0)), direction: rng.gen()};
...
fn update(mut commands: Commands, mut query: Query<(&mut Dots, &mut Transform)>, lines: Query<Entity, With<Lines>>, windows: ResMut<Windows>, time: Res<Time>) {
    let win = windows.get_primary().expect("no primary window");
    for mut i in query.iter_mut() {
        ...
        if i.0.pos.0.abs()>win.width()/2.0+MAX_DIST || i.0.pos.1.abs()>win.height()/2.0+MAX_DIST {
            let mut rng = rand::thread_rng();
            let num = (rng.gen::<f32>()*((win.width()+MAX_DIST*2.0)*2.0+(win.height()+MAX_DIST*2.0)*2.0)+1.0).floor();
            let mut x = 0.0;
            let mut y = 0.0;
            ...
            else if num <= (win.width()+MAX_DIST*2.0)*2.0+(win.height()+MAX_DIST*2.0)*2.0 {
                y = num-((win.width()+MAX_DIST*2.0)*2.0+win.height()+MAX_DIST*2.0)-win.height()+MAX_DIST;
